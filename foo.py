...
def run_comparison(image_path):
    ...
    print(f"Starte Vergleich fuer: {img_path.name}")
    print(f"Originalgroesse: {w_orig} x {h_orig}")
    ...
    print(f"Skalierte Groesse: {w_low} x {h_low}")
    ...
    for key in scores_orig.keys():
        ...
        marker = "!!" if diff > 0.05 else "ok"
        print(f"{key:<20} | {val_orig:.4f}   | {val_low:.4f}   | {diff:.4f} {marker}")
